...
### v1.1 (planned): Google Health API support

The Fitbit Web API (the recipe's origin incident) is migrating to the [Google Health API](https://developers.google.com/health/migration) on Google OAuth 2.0, with the legacy API turned down September 2026. Google OAuth 2.0 uses a different refresh model from the single-use providers this recipe targets today, so v1.1 will add a tested Google Health / Google OAuth 2.0 path, built and verified against a live integration, not a drop-in rename. Targeted for after the new API stabilises (Google advises against launching integrations before end of May 2026 while breaking changes land).
